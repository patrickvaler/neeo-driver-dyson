...
module.exports = class DysonPure extends EventEmitter {
  ...
  speedUp() {
    if (this._state.fnsp > 9) {
      debug('Fan Speed is 10, can not speed up');
      return;
    }
  ...
  speedDown() {
    if (this._state.fnsp < 2) {
      debug('Fan Speed is 1, can not speed down');
      return;
    }
  ...
  _tranformDysonResponse(state) {
    return {
      ...
      fnsp: state.fnsp === 'AUTO' ? 0 : parseInt(state.fnsp)
    };
  }
