...
function _initDevice(deviceId) {
  ...
  if (device) {
    ...
    device.on('state-changed', () => {
      ...
      _sendPowerStateUpdateToBrain(device.getPowerState());
      _sendStateUpdateToBrain(deviceId, Switches.POWER_SWITCH, device.getPowerState());
      _sendStateUpdateToBrain(deviceId, Switches.AUTO_SWITCH, device.getAutoState());
    });
    device.connect();
  }
}
