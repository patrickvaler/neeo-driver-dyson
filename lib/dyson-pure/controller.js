...
function onButtonPressed(name, deviceId) {
  ...
  switch (name) {
    ...
    case Buttons.AUTO_OFF:
      device.autoOff();
      break;
    case Buttons.NIGHT_MODE_ON:
      device.nightModeOn();
      break;
    case Buttons.NIGHT_MODE_OFF:
      device.nightModeOff();
      break;
...
const switchCallbacks = {
  ...
  [Switches.NIGHT_MODE_SWITCH]: nightModeSwitchCallback,
...
function _initDevice(deviceId) {
  ...
  if (device) {
    ...
    device.on('state-changed', () => {
      ...
      _sendStateUpdateToBrain(deviceId, Switches.NIGHT_MODE_SWITCH, device.isNightMode());
    });
    device.connect();
  }
}
